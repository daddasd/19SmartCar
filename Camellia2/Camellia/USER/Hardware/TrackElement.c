...
int Obstacle_Distance1 = 1800; // 第一次直行的距离
int Obstacle_Distance2 = 2548; // 第二次直行的距离
...
int Yu_Distance = 3600;	 // 预进环岛距离
int Yu_Speed_Diff = 800; // 打角的速度差异

int Chu_Speed = 1000;	  // 预出环岛速度
int Chu_Distance = 2800;  // 预出环岛距离
...
int End_Chu_Distance = 2000; // 出环岛距离
...
// 			pwm = Speed_pid_Out(60, Sum_Pulse);
...
// 		Obstacle_Flag = 0;
// 		Inductance_Error = 0;
// 		Obstacle_State = NO_LObstacle; // 防止误判			 // 打开正常循迹
// 		Elements_Num++;				   // 元素表++
// 		speed = 45;
// 		Buzzer_OFF;
// 		break;
...
void RRoundabout(void)
{
	...
	case In_Ring:
		...
		if (time > 200)
		{
			if ((L3_NOR_ADC + R3_NOR_ADC) >= 140 || M1_NOR_ADC > 75||R1_NOR_ADC>90)
			{
	...
	case Ready_Out_Ring:
		...
		if (bmq_jifen >= Chu_Distance + 1400)
		{
			bmq_jifen = 0;
			Distance_Flag = 1;
			// Motor_PWM(0, 0); // 速度清零
...
void LRoundabout(void)
{
	...
	case In_Ring:
		...
		if (time > 200)
		{
			if ((L3_NOR_ADC + R3_NOR_ADC) >= 140 || M1_NOR_ADC > 75 || L1_NOR_ADC > 90)
			{
...
void Ramp(void)
{
	...
	static int Flag = 0;
	if ((dl1a_distance_mm < 300 &&M1_NOR_ADC > 75)||Flag == 1)
	{
		Flag = 1;
		Ramp_Time++;
		Buzzer_ON;
		if (Ramp_Time > 50&&Ramp_Time<250)
		{
			speed = 25;//下坡减速
		}
		else if(Ramp_Time>=500)
		{
			speed = 70;
			Elements_Num++;
			Flag = 0;
...
void Tracking(int Set_speed)
{
	...
	{
		count++;
		dir_out = DirControl(-Inductance_Error*0.80);
		if (count >= 10)
		{
		...
		LMotor_PWM = LALL_PWM;
		RMotor_PWM = RALL_PWM;
		// LALL_PWM = limit(LALL_PWM, 9500);
		// RALL_PWM = limit(RALL_PWM, 9500);
