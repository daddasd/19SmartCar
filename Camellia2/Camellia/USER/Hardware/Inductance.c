...
float NOR_VAL[5] = {0};
float AD_NOR_VAL[5] = {0};	// 归一化值
int16 Inductance_Error = 0; // 电感误差
uint16 ADC_MIN[5] = {156, 252,150,180, 149};
uint16 ADC_MAX[5] = {3020, 3520,  3520,  3520, 3020}; // 测出的电感最大，与最小值从左到右
...
int16 NORMALIZATION_TRACKING_ADC(void)
{
	...
	for (i = 0; i < 10; i++) // 读取十次电感
	{
		...
		ad_value[2][i] = adc_once(ADC_P01, ADC_12BIT);
		ad_value[3][i] = adc_once(ADC_P03, ADC_12BIT);
		ad_value[4][i] = adc_once(ADC_P02, ADC_12BIT);
	...
	NOR_VAL[2] = (float)(M1 - ADC_MIN[2]) / (float)(ADC_MAX[2] - ADC_MIN[2]);
	NOR_VAL[3] = (float)(R2 - ADC_MIN[3]) / (float)(ADC_MAX[3] - ADC_MIN[3]);
	NOR_VAL[4] = (float)(R1 - ADC_MIN[4]) / (float)(ADC_MAX[4] - ADC_MIN[4]);
	...
	M1_NOR_ADC = AD_NOR_VAL[2];
	R2_NOR_ADC = AD_NOR_VAL[3];
	R1_NOR_ADC = AD_NOR_VAL[4];
...
void show_val(void)
{
	...
	oled_p6x8str(0, 2, "M1:");
	oled_uint16(15, 2, M1_NOR_ADC);

	oled_p6x8str(55, 2, "Err:");
	oled_int16(80, 2, Inductance_Error);

	////-----------------陀螺仪角速度------------------------//
	oled_p6x8str(0, 3, "G_z");
	oled_int16(20, 3,mpu6050_gyro_z);
	oled_p6x8str(60, 3, "Dis");
	oled_int16(80, 3, dl1a_distance_mm);
