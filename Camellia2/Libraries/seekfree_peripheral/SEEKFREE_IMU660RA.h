...
//=====================================================软件 IIC 驱动====================================================
#define IMU660RA_SCL_PIN (P37)		// 软件 IIC SCL 引脚 连接 IMU660RA �? SCL 引脚
#define IMU660RA_SDA_PIN (P36)		// 软件 IIC SDA 引脚 连接 IMU660RA �? SDA 引脚
#define IMU660RA_SOFT_IIC_DELAY (0) // 软件 IIC 的时钟延时周�? 数值越�? IIC 通信速率越快
//=====================================================软件 IIC 驱动====================================================
#else
//=====================================================软件 SPI 驱动====================================================
...
#define IMU660RA_ACC_SAMPLE (0x02) // 加速度计量�?
// 设置�?:0x00 陀螺仪量程�?:±2000dps     获取到的陀螺仪数据 除以 16.4       可以转化为带物理单位的数�? 单位为：°/s
// 设置�?:0x01 陀螺仪量程�?:±1000dps     获取到的陀螺仪数据 除以 32.8       可以转化为带物理单位的数�? 单位为：°/s
// 设置�?:0x02 陀螺仪量程�?:±500 dps     获取到的陀螺仪数据 除以 65.6       可以转化为带物理单位的数�? 单位为：°/s
// 设置�?:0x03 陀螺仪量程�?:±250 dps     获取到的陀螺仪数据 除以 131.2      可以转化为带物理单位的数�? 单位为：°/s
// 设置�?:0x04 陀螺仪量程�?:±125 dps     获取到的陀螺仪数据 除以 262.4      可以转化为带物理单位的数�? 单位为：°/s

#define IMU660RA_GYR_SAMPLE (0x00) // 陀螺仪量程
// 设置�?:0x00 加速度计量程为:±2g         获取到的加速度计数�? 除以 16384   可以转化为带物理单位的数�? 单位：g(m/s^2)
// 设置�?:0x01 加速度计量程为:±4g         获取到的加速度计数�? 除以 8192    可以转化为带物理单位的数�? 单位：g(m/s^2)
// 设置�?:0x02 加速度计量程为:±8g         获取到的加速度计数�? 除以 4096    可以转化为带物理单位的数�? 单位：g(m/s^2)
// 设置�?:0x03 加速度计量程为:±16g        获取到的加速度计数�? 除以 2048    可以转化为带物理单位的数�? 单位：g(m/s^2)

extern int16 imu660ra_gyro_x, imu660ra_gyro_y, imu660ra_gyro_z; // 三轴陀螺仪数据      gyro (陀螺仪)
extern int16 imu660ra_acc_x, imu660ra_acc_y, imu660ra_acc_z;	// 三轴加速度计数�?     acc (accelerometer 加速度�?)

void imu660ra_get_acc(void);					  // 获取 IMU660RA 加速度计数�?
void imu660ra_get_gyro(void);					  // 获取 IMU660RA 陀螺仪数据
float imu660ra_acc_transition(int16 acc_value);	  // �? IMU660RA 加速度计数据转换为实际物理数据
float imu660ra_gyro_transition(int16 gyro_value); // �? IMU660RA 陀螺仪数据转换为实际物理数�?
uint8 imu660ra_init(void);						  // 初始�? IMU660RA
